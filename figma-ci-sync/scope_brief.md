...
- In-scope: canonical DTCG token sources; theme structure; component recipe manifest; token and recipe validation; build transforms and generated artifacts; Next.js and Storybook consumption; Figma sync rails via plugin-based variable import in the short term and an app-owned API rail later; local and CI drift checks; migration off hand-edited runtime token CSS.
- Out-of-scope: a full component-library rollout; a monorepo/workspace conversion; two-way Figma authoring as the normal workflow; mandatory API-driven Figma automation unless it becomes a hard requirement; broad visual redesign work unrelated to the token system.
...
- Constraints: the repo is currently single-package; `just preflight`, `just check`, and LOC limits are hard gates; the current app surface is dark-themed; Storybook is already part of the contract layer; short-term Figma proof should work through a plugin rail before the repo depends on a service-owned API rail.
- External systems / dependencies: Figma, Figma Plugin API, optional repo-owned Figma OAuth app plus Variables REST API, Style Dictionary, `@tokens-studio/sd-transforms`, Storybook, CI, optional Chromatic.
- Known unknowns / risks: final source-directory layout if the repo later adopts workspaces; when additive themes beyond `dark` are worth standardizing; the availability and ergonomics of a supported OAuth-app-backed Variables rail if parity promotion is revisited after v1.
...
- V1 Figma proof uses a plugin-based import/materialization rail against `design-tokens/dist/figma/tokens.json`; Figma does not push values back to the repo in v1. Tokens Studio may be used only as a temporary carrier while the repo moves toward a repo-owned plugin or importer. This matches `threading.md` (`CT-7`) and `threaded-seams/seam-5-figma-sync-rail/slice-1-sync-contract-and-policy-baseline.md`.
- V1 parity policy has two allowed states only: `deferred` and `required`. It starts at `deferred` and can move to `required` only after a supported OAuth-app-backed Variables rail exists, the pilot sync ledger is stable, and `SEAM-6` owns a deterministic parity gate. See `threaded-seams/seam-5-figma-sync-rail/seam.md`.
...
- When an OAuth-app-backed Variables rail becomes available, is the team ready to promote parity from `deferred` to `required` without reopening the source-of-truth model?
