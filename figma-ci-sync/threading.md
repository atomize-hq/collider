...
- Definition: `design-tokens/dist/figma/tokens.json` is the Figma-facing export. The short-term proof transport is `plugin-import-manual`, meaning a Figma plugin or importer materializes repo-approved variables from the generated artifact while repo PRs remain the only path to change canonical values. The preferred long-term hardened transport is `rest-variables-oauth`, meaning a repo-owned OAuth app writes the same approved artifact into Figma through the Variables API once access, scopes, and governance are in place. Tokens Studio is not part of the permanent contract and may be used only as a temporary pilot carrier. The seam-owned pilot ledger at `src/figma/sync-ledger.json` has fixed root keys `ledgerVersion`, `scope`, `name`, `links`, `status`, and `drift`; `ledgerVersion="1"`, `scope="figma-pilot"`, `status.syncMode` is `plugin-import-manual` or `rest-variables-oauth`, `status.artifactPath="design-tokens/dist/figma/tokens.json"`, `status.artifactGitSha` stores the reviewed commit SHA for the import/materialization attempt, `status.themeIds` and `status.themeMapping` record imported themes, and `status.parityMode` has two allowed values only, `deferred` and `required`. When `parityMode=deferred`, `status.parityDeferredReason` is required; when `parityMode=required`, that field is forbidden. `drift` is always an array of explicit exception objects; the happy path uses `[]`.
- Versioning/compat: write-back from Figma into canonical repo source remains forbidden; any bidirectional model would be a separate explicit policy change because it changes the system-of-record model.
...
- Figma sync posture for v1 is `plugin-import-manual`, with `parityMode` fixed to `deferred` until a supported API-backed parity rail is real and explicitly adopted.
