...
- **Acceptance criteria**:
  ...
  - The validator rejects contradictory states such as `syncMode=read-only-url` with status metadata claiming Figma authored the latest canonical change.
...
- Implement:
  ...
  - Require branch-specific metadata such as a defer reason when parity is not enforced.
- Test:
