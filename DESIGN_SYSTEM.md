...
| ChevronToggle (collapsed, not hovered) | 0×16px *(zero-width — layout shift intended)* |
| FileIcon | 16×16px |
...
| VerbToken | 34–76×18px *(width varies by verb)* |
...
| HintGroup | 80–137×18px *(width varies by content)* |
| GitFileRow | 370×24px |
...
*(Previously called RunBadge — BadgeChip is the canonical Figma component name.)*
