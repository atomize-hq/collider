...
describe('flattenTokenDocument', () => {
  it('produces a deterministic sorted leaf list from the token artifact', () => {
    ...
    expect(leaves.at(-1)?.name).toBe('spacing/8');
  });
  ...
  it('parses color values from hex, hex8, rgb(), and rgba()', () => {
    ...
    expect(byName.get('core/color/hex')?.value).toEqual({ r: 1, g: 0, b: 0, a: 1 });
    ...
    expect(byName.get('core/color/rgb')?.value).toEqual({ r: 0, g: 0, b: 1, a: 1 });
