...
function walkTokenNode(node: unknown, pathSegments: string[], leaves: TokenLeaf[]) {
  ...
  if (Object.prototype.hasOwnProperty.call(node, '$value')) {
    ...
    if (typeof tokenType !== 'string' || !supportedTokenTypes.has(tokenType as TokenLeafType)) {
      throw new Error(`unsupported token type ${String(tokenType)} at ${pathSegments.join('.')}`);
    }
...
function normalizeTokenValue(tokenType: TokenLeafType, value: unknown): FigmaValue {
  switch (tokenType) {
    case 'color':
...
function parseColorValue(value: unknown): FigmaColor {
  ...
  if (normalized.startsWith('#')) {
    ...
    if (!/^[0-9a-fA-F]{6}([0-9a-fA-F]{2})?$/.test(hex)) {
      throw new Error(`color token value ${value} must use #RRGGBB or #RRGGBBAA`);
    }
  ...
  const channel = (input: string) => {
    ...
    if (number < 0 || number > 255) {
      throw new Error(`color token value ${value} must keep RGB channels between 0 and 255`);
    }
    return number / 255;
  };
  ...
  if (alpha < 0 || alpha > 1) {
    throw new Error(`color token value ${value} must keep alpha between 0 and 1`);
  }
