...
describe('readDefaultThemeId', () => {
  it('reads the theme id the artifact declares', () => {
    expect(
      readDefaultThemeId({ $extensions: { 'com.atomizehq.collider': { themeId: 'light' } } })
    ).toBe('light');
  });

  it('falls back to dark when the extension is absent or malformed', () => {
    expect(readDefaultThemeId({})).toBe('dark');
    expect(readDefaultThemeId(null)).toBe('dark');
    expect(readDefaultThemeId({ $extensions: { 'com.atomizehq.collider': { themeId: 7 } } })).toBe(
      'dark'
    );
  });
});
...
describe('buildExpectedVariables', () => {
  it('carries every theme in the real artifact', () => {
    const expected = buildExpectedVariables(loadArtifact());
    expect(expected.defaultThemeId).toBe('dark');
    expect(expected.themeIds).toEqual(['dark', 'light']);
    expect(expected.variables).toHaveLength(176);
  ...
  it('applies $themeOverrides per theme and inherits the base value otherwise', () => {
    const expected = buildExpectedVariables({
      $extensions: { 'com.atomizehq.collider': { themeId: 'dark' } },
      $themeOverrides: {
    ...
    const byName = new Map(expected.variables.map((entry) => [entry.name, entry]));
    expect(byName.get('bg/base')?.valuesByTheme.dark).toEqual({ r: 0, g: 0, b: 0, a: 1 });
    expect(byName.get('bg/base')?.valuesByTheme.light).toEqual({ r: 1, g: 1, b: 1, a: 1 });
  ...
  it('does not duplicate the default theme when $themeOverrides also names it', () => {
    const expected = buildExpectedVariables({
      $extensions: { 'com.atomizehq.collider': { themeId: 'dark' } },
      $themeOverrides: { dark: { bg: { $type: 'color', $value: '#111111' } } },
...
describe('compareFigmaVariables', () => {
  it('reports no drift when the collection matches the artifact', () => {
    const expected = buildExpectedVariables(loadArtifact());
    const report = compareFigmaVariables(expected, observedFromExpected(expected));

    expect(report.ok).toBe(true);
    expect(report.findings).toEqual([]);
    expect(report.expectedVariableCount).toBe(176);
    expect(report.observedVariableCount).toBe(176);
  });
  ...
  it('flags a variable someone added in Figma', () => {
    ...
    observed.variables.push({
      ...
      resolvedType: 'COLOR',
      valuesByMode: { dark: { r: 1, g: 0, b: 0, a: 1 }, light: { r: 1, g: 0, b: 0, a: 1 } },
    });

    const report = compareFigmaVariables(expected, observed);
    expect(report.findings).toContainEqual(
      expect.objectContaining({ code: 'UNEXPECTED_VARIABLE', name: 'brand/experimental' })
    );
  });
  ...
  it('flags a value edited in Figma and names both sides in hex', () => {
    const expected = buildExpectedVariables(loadArtifact());
    const observed = observedFromExpected(expected);
    const target = observed.variables.find((entry) => entry.resolvedType === 'COLOR')!;
    target.valuesByMode.dark = { r: 1, g: 0, b: 0, a: 1 };

    const report = compareFigmaVariables(expected, observed);
    const finding = report.findings.find((entry) => entry.code === 'VALUE_MISMATCH');
  ...
  it('reports an alias binding separately from a plain value mismatch', () => {
    const expected = buildExpectedVariables(loadArtifact());
    const observed = observedFromExpected(expected);
    observed.variables[0]!.valuesByMode.dark = { type: 'VARIABLE_ALIAS', id: 'VariableID:1:2' };

    const report = compareFigmaVariables(expected, observed);
    expect(report.findings).toContainEqual(
      expect.objectContaining({ code: 'ALIAS_BINDING', name: observed.variables[0]!.name })
    );
    expect(report.findings.some((entry) => entry.code === 'VALUE_MISMATCH')).toBe(false);
  });
  ...
  it('flags a resolvedType change and stops checking that variable', () => {
    ...
    const report = compareFigmaVariables(expected, observed);
    const forTarget = report.findings.filter((entry) => entry.name === target.name);
  ...
  it('flags a missing mode once rather than once per variable', () => {
    ...
    const report = compareFigmaVariables(expected, observed);
    expect(report.findings.filter((entry) => entry.code === 'MISSING_MODE')).toEqual([
      expect.objectContaining({ mode: 'light' }),
    ]);
    expect(report.findings.some((entry) => entry.mode === 'light' && entry.name)).toBe(false);
  });
  ...
  it('flags a mode added in Figma', () => {
    ...
    const report = compareFigmaVariables(expected, observed);
    expect(report.findings).toContainEqual(
      expect.objectContaining({ code: 'UNEXPECTED_MODE', mode: 'high-contrast' })
    );
  });
  ...
  it('orders findings deterministically', () => {
    const expected = buildExpectedVariables(loadArtifact());
    const observed = observedFromExpected(expected);
    observed.variables.push({ name: 'zz/extra', resolvedType: 'STRING', valuesByMode: {} });
    observed.variables.push({ name: 'aa/extra', resolvedType: 'STRING', valuesByMode: {} });
    observed.modeNames.push('extra-mode');

    const codes = compareFigmaVariables(expected, observed).findings.map((entry) => entry.code);
...
describe('valuesEqual', () => {
  ...
  it('treats a missing alpha channel as opaque', () => {
    expect(valuesEqual({ r: 0, g: 0, b: 0, a: 1 }, { r: 0, g: 0, b: 0 })).toBe(true);
  });
});
...
describe('formatDriftReport', () => {
  it('states plainly when nothing drifted', () => {
    const expected = buildExpectedVariables(loadArtifact());
    const text = formatDriftReport(compareFigmaVariables(expected, observedFromExpected(expected)));
