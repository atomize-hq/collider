/**
 * Read-only comparison between the published Figma artifact and what a Figma
 * file actually contains.
 *
 * The repo is canonical: `design-tokens/src/tokens/` is the only authoring
 * surface, and `design-tokens/dist/figma/tokens.json` is its published
 * projection. This module never converts Figma back into DTCG — it only
 * reports where the two disagree, so an intentional Figma-side edit surfaces
 * as a reviewable finding instead of being silently written into canon.
 *
...
import {
  ...
  type FigmaValue,
} from './figma-token-mapping';

export const fallbackThemeId = 'dark';
...
export function readDefaultThemeId(payload: unknown): string {
 ...
 */
export function buildExpectedVariables(payload: unknown): ExpectedVariableSet {
  ...
  const rawOverrides = (payload as { $themeOverrides?: Record<string, unknown> }).$themeOverrides;
  if (rawOverrides) {
    for (const themeId of Object.keys(rawOverrides)) {
      const leaves = flattenTokenDocument(rawOverrides[themeId]);
      overridesByTheme.set(themeId, new Map(leaves.map((leaf) => [leaf.name, leaf.value])));
    }
  }
  ...
  const variables = base.map((leaf) => {
    ...
    for (const themeId of themeIds) {
      valuesByTheme[themeId] = overridesByTheme.get(themeId)?.get(leaf.name) ?? leaf.value;
    }
...
export function compareFigmaVariables(
  ...
  const observedByName = new Map(observed.variables.map((entry) => [entry.name, entry]));
...
export function formatDriftReport(report: DriftReport): string {
  const header = [
    report.ok ? 'Figma drift: NONE' : `Figma drift: ${report.findings.length} finding(s)`,
  ...
  if (report.ok) {
    return [...header, '', 'The Figma file matches the published artifact.'].join('\n');
  }
  const lines = report.findings.map(
    (finding) => `  [${finding.code}] ${finding.name ?? finding.mode ?? ''} — ${finding.detail}`
  );
