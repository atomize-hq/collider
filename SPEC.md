...
| S2  | The token gate still bites            | Perturb the artifact **in an isolated worktree** (preflight step 1 regenerates it in place — see §6). Require a rail-specific diagnostic, not any non-zero exit. Also cover: tool missing, input missing, malformed JSON, mismatched expectations.                                                                                           |
...
- **The full normalized rail mapping** — ordered names, types, collection and namespace,
  per-theme values — not the five summary fields.

Both are the reference for S3 and S4. A baseline regenerated by the new code is not evidence.

### 7.2 The S2 trap, and how to avoid it
...
**regenerated before any verify step observes it**. Perturb-and-run would have passed while
proving nothing.

Verify in an isolated worktree or equivalent fixture setup, confirm the run fails **for the
intended invariant**, and restore reliably. Do not accept "command not found", an unrelated
Storybook failure, or an earlier gate tripping as evidence that the token gate works.
