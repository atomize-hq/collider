...
├── skills/                       # 3 tooling skills as data (not the 2 ai-elements ones)
...
component sources are `src/components/ai-elements/` and were never in scope. The question was
therefore **cohesion, not licensing**, and it is settled: **three skills ship** —
`sync-quality-governor`, `stage-1-foundation-primitives-system`, and
`storybook-rigorous-spec-system` (which owns four of the five schemas that already moved).
`ai-elements` and `ai-elements-plate-builder` stay with the consumer, **and no shipped skill may
reference them** — enforced by `pack-check`, along with the rule that nothing shipped as data may
name a consumer. See [`docs/ds-skills-disclosure-review.md`](docs/ds-skills-disclosure-review.md)
§6.
...
identity is empty **or still a placeholder** — a bootstrap that does not know what it is must not
guess, and the ungenerated template is the likeliest way one ends up in that state.

The bootstrap takes **no flags at all**, because `curl … | bash` has no argv to read them from.
The two things a caller legitimately varies are environment variables: `DS_SKILLS_PREFIX` (CI
needs a job-local prefix, §10.6) and `DS_SKILLS_BASE_URL` (a mirror). The second is safe
precisely because the payload digests are baked and enforced: an override that cannot change
what gets installed is a mirror, not a redirect.
...
```json
{
  ...
  "bootstrapPowershell": { "asset": "install.ps1", "sha256": "…" },
  "checksums": { "asset": "SHA256SUMS", "sha256": "…" },
  "assets": {
...
`bootstrapPowershell` and `checksums` were added at T14. The two installers are a matched pair
(§10.6), so pinning only the bash one would leave the Windows path unanchored; and pinning
`SHA256SUMS` lets a consumer notice a republished checksum list even though the baked digests
already make it non-authoritative. The file is generated by `scripts/release/stage.mjs` from the
staged bytes — staging twice produces identical digests, which is what lets "the tested bytes"
and "the published bytes" be one claim rather than two.
...
Assets are per-platform as a **selection contract**: an unlisted OS/arch pair fails with the
list, rather than downloading something that will not run.
