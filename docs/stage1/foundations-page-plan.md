...
> **STATUS (2026-09-01): COMPLETE — all four phases landed.**
>
> - `c6d5e5f` — Phase 0b/0c: core rungs `neutral.400` / `neutral.600` / `violet.400`, text ladder re-valued, four a11y suppressions retired. Also repaired the accessibility gate itself, which had never actually run.
> - `b53ed80` — Phase 0a/0d: accent flattened onto the theme rail, `theme.tokens.json` removed. Breaking migration, recorded in the commit body.
> - `051a4b0` — Phase 1: 176 variables published and verified; ledger and proof re-pinned.
> - Phase 2: six frames built on a new **Foundations** page, **562 bound paints and zero unbound** — the whole page follows the mode switcher. Builders committed under `figma/foundations/`, generated by `pnpm figma:foundations:build`, and idempotent.
> - Phase 3: all four legacy swatch frames deleted. They were not merely redundant — their labels were hardcoded hex that the 0b re-value had made **wrong**, and they bound a dead fork collection. Primitives canvas is now 33 component seeds and nothing else.
>
> **What the page surfaced on its first render** (each is a real finding, not a build defect):
>
> - `accent/sidebar-primary-foreground` is **3.46:1 on its own accent surface in dark** — below AA for body text. Only visible once foreground tokens were measured against their pairing instead of the page ground.
> - `text/tertiary` reads `AA-large only` in dark, as designed — it is the deliberate dim step and is annotated as such.
> - Border and status-strip ratios are printed **without** a verdict: WCAG 1.4.11 exempts decorative dividers, and whether a status strip needs 3:1 depends on whether it is the sole carrier of state. Asserting pass/fail there would have been the page overstating its own authority.
