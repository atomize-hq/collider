...
- **User/system value**: prove a designer can materialize the repo-approved artifact into one Figma test file and apply the supported theme contract without manual token entry.
- **Scope (in/out)**:
  - In: one team-owned Figma pilot file; the exact plugin/import setup flow for consuming `design-tokens/dist/figma/tokens.json`; ledger updates that record which themes and statuses were proven.
  - Out: broader team rollout across many Figma files; API-backed parity; CI ownership or gate promotion.
- **Acceptance criteria**:
  - The pilot Figma file can materialize the canonical export without hand-entering token values.
...
- **Verification**: follow the pilot setup doc from a clean starting point; import/materialize the artifact into the pilot Figma file; inspect that variables or styles materialize for the required theme; update the ledger with the observed result.
...
#### S2.T1 — Configure and document the pilot plugin import flow

- **Outcome**: a maintainer can use one documented plugin/import path to materialize the canonical export in a Figma test file.
- **Inputs/outputs**:
  - Inputs: `src/figma/README.md`; `design-tokens/dist/figma/tokens.json`; the chosen default sync mode from `S1.T1`.
  - Outputs: `src/figma/pilot-setup.md` capturing the supported plugin/import path, artifact handoff format, any authentication expectations, and the exact materialization steps.
- **Implementation notes**: document the plugin path the team actually intends to support. The short-term proof should work through a repo-owned or OSS-backed plugin/importer rather than depending on Tokens Studio. If a temporary carrier is still used in practice, call it out as transitional rather than normative.
- **Acceptance criteria**:
  - A maintainer following `src/figma/pilot-setup.md` can materialize the canonical artifact in the pilot file.
...
- **Risk/rollback notes**: importer/plugin UX drift is the main risk; keep the guide versioned and concise so the team can update field names or input steps without changing policy.
...
- `status.themeMapping` must use one object per theme with the shape `{ "themeId": "<ct-2-theme-id>", "figmaMode": "<observed Figma mode or collection name>" }`.
