...
| T17 switched executable callers but nothing activated the skills — stale Markdown and agent discovery paths survive an otherwise complete migration.                                                                                                                            | Skill cutover is explicit T17 work, staged at T16, decided at T5. **Landed, and the risk was real**: the forked `schemas/` sat at ledger v2 while the rail had moved to v3. |
...
- [x] T10: Move the rail modules under `src/figma/` — `884cbb2`; copies not moves, pinned against pre-move outputs, which caught an order-dependent reason-code branch
...
- [x] The pack has a lint gate and the LOC guard — `ds-skills` `37a9971`. **This item had no owning
...
- [x] T17: Activate Collider's callers and delete what they supersede — every rail caller now invokes the pinned CLI and Collider owns no rail executable. The ledger is v3 with its `publication` binding, `@atomize-hq/figma-token-rail` is gone from `package.json` and the lockfile, and the three release-owned skills plus `schemas/` and `templates/` are linked from the install rather than forked — the fork was already stale at ledger v2. CT-7B is gated for the first time. **S1, S2, S3 and S4 all hold**, and `just preflight` is green from a clean install. Three inventory corrections found by doing the cutover rather than reading it, recorded in disposition inventory §9
...
- **Does `.agents/skills/` become generated output or stay tracked?** Blocks the rest of T11.
  ~~Which of the eight skills belong?~~ **Settled 2026-09-06** — three move
  (`sync-quality-governor`, `stage-1`, `storybook-rigorous-spec-system`); `ai-elements` and
  `ai-elements-plate-builder` stay with the consumer, and no shipped skill may reference them.
