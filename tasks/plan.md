...
| **A required check that never ran.** Job 7 is fork-conditional and job 8 `needs:` it, so the CT-8B rail already skips on fork PRs — and a skipped job reports success.                                           | **High**                          | T16a names workflow file, job id, check context, triggers and binding, records when each job does **not** execute, and places the replacement gate upstream of the fork guard.                                                                                       |
