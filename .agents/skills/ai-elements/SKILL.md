...
> **REPO RULE (Collider):** the registry CLI is for **new** components only. `src/components/ui`
> and `src/components/ai-elements` are vendored copies this repo owns and has edited; re-running
> `add` over an existing file reverts those edits without failing a test. Never pass
> `--overwrite`. Divergences and the upstream API we depend on are recorded in
...
> **REPO RULE (Collider):** there is no `tailwind.config.js`/`.ts` here — it was dead config and
> was deleted. Tailwind v4 is wired through `@theme inline` in `src/app/globals.css`, which also
> hand-authors the shadcn role bridge onto the Collider `--color-*` tokens and repoints Tailwind's
> `dark:` variant at `data-theme`. Fix theming there, not in a config file you would have to
> recreate.
