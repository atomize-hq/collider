 ...
 */
const scriptPath = fileURLToPath(new URL('./validate-artifact.mjs', import.meta.url));
const schemaDir = fileURLToPath(new URL('../schemas/', import.meta.url));
const profilePath = fileURLToPath(new URL('../profiles/collider.json', import.meta.url));
const repoLedgerPath = fileURLToPath(
  new URL('../../../src/figma/sync-ledger.json', import.meta.url)
);
...
describe('validate-artifact CLI', () => {
  ...
  it('resolves a local $ref and lets sibling keys override the target', () => {
    ...
    expect(
      run(schema, fixture('ref-ok.json', { loose: 'anything', strict: 'only-this' })).status
    ).toBe(0);

    const violation = run(schema, fixture('ref-bad.json', { loose: 'anything', strict: 'other' }));
  ...
  it('applies repo vocabulary only when a profile is supplied', () => {
    // artifact.path is an open string in the portable schema; Collider's profile
    // pins it to one literal. Same instance, two verdicts — that split is the
    // whole point of keeping schemas portable and vocabulary in profiles.
    const real = JSON.parse(fs.readFileSync(repoLedgerPath, 'utf8'));
    const ledger = fixture('ledger.json', {
      ...real,
      artifact: { ...real.artifact, path: 'some/other/path.json' },
    });
    ...
    expect(profiled.stderr).toContain('design-tokens/dist/figma/tokens.json');
  });
