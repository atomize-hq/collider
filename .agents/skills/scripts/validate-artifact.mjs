...
function main() {
  ...
  const errors = validate(instance, schema, { root: schema, profile, path: '' });
  ...
  console.log(`✓ ${schema.title ?? path.basename(schemaPath)}: ${path.resolve(instancePath)}`);
}
...
function derefPointer(ref, root, where) {
  if (ref === '#') return root;
  if (!ref.startsWith('#/')) fail(`${label(where)}: only local $ref is supported, got "${ref}"`);
  let target = root;
  ...
  if (target === undefined) fail(`${label(where)}: $ref "${ref}" does not resolve`);
  return target;
}
...
function checkType(value, node, ctx, errors) {
  ...
  if (!types.some((type) => matchesType(value, type))) {
    errors.push(`${label(ctx.path)} must be ${types.join(' or ')}, got ${describe(value)}`);
  }
}
...
function checkEnumAndConst(value, node, ctx, errors) {
  ...
  if (Object.prototype.hasOwnProperty.call(node, 'const') && !deepEqual(node.const, value)) {
    errors.push(
...
function checkString(value, node, ctx, errors) {
  ...
  if (typeof node.minLength === 'number' && value.length < node.minLength) {
    errors.push(`${label(ctx.path)} must be at least ${node.minLength} character(s)`);
  }
  if (typeof node.pattern === 'string' && !new RegExp(node.pattern, 'u').test(value)) {
    errors.push(`${label(ctx.path)} must match /${node.pattern}/, got ${JSON.stringify(value)}`);
  }
}
...
function checkArray(value, node, ctx, errors) {
  ...
  if (node.items !== undefined) {
    for (const [index, entry] of value.entries()) {
      errors.push(...validate(entry, node.items, { ...ctx, path: `${ctx.path}[${index}]` }));
    }
  }
}
...
function checkObject(value, node, ctx, errors) {
  ...
  for (const [key, subSchema] of Object.entries(properties)) {
    if (Object.prototype.hasOwnProperty.call(value, key)) {
      errors.push(...validate(value[key], subSchema, { ...ctx, path: join(ctx.path, key) }));
    }
  }
  ...
  if (node.additionalProperties !== undefined) {
    for (const key of keys) {
      ...
      if (node.additionalProperties === false) {
        ...
        errors.push(
          ...validate(value[key], node.additionalProperties, { ...ctx, path: join(ctx.path, key) })
        );
      }
    }
  }
...
function checkCombinators(value, node, ctx, errors) {
  ...
  if (Array.isArray(node.oneOf)) {
    ...
    if (passing === 0) {
      ...
      const reasons = results.flat().map((reason) => reason.replace(/^\S+ /, ''));
      errors.push(
        `${label(ctx.path)} matched no allowed form: ${[...new Set(reasons)].join('; ')}`
      );
    } else if (passing > 1) {
      errors.push(`${label(ctx.path)} is ambiguous — it matched ${passing} allowed forms`);
    }
  }
  ...
  if (node.if !== undefined) {
    const branch = validate(value, node.if, ctx).length === 0 ? node.then : node.else;
    if (branch !== undefined) {
