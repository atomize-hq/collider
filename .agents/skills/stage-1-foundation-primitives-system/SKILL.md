...
- establishing AI Elements and Plate conventions
...
- `src/components/ui/` — design-system primitives (shadcn-derived; 23 of them today)
- `src/components/ai-elements/` — local AI Elements wrappers
- `src/components/editor/` — Plate wrappers, kits, plugins (planned; Plate is not installed
  today and this directory does not yet exist)
- `src/features/` — feature-local composition
- `src/bridge/` — typed Tauri bridge
...
**Status in this repo: largely done.** `src/components/ui` already holds 23 shadcn-derived
primitives, pulled in as dependencies of the ai-elements loops rather than as a deliberate
wave. Treat the list below as a coverage checklist to audit against, not a greenfield plan.

This layer is also where **component recipes (CT-3) attach**. The recipe shape — variantAxes /
slots / states / defaults / fallbacks — is a CVA atom shape; it does not describe an
ai-elements organism. Organisms get tokens transitively through the atoms they compose and
never need recipes of their own. Only 5 of the 23 primitives declare CVA variants today, so
"author recipes for the primitives" is partly recording what exists and partly designing a
variant surface that does not.
...
`src/components/ui` (shadcn) and `src/components/ai-elements` (ai-elements) are vendored by
copy. Neither is a dependency you can bump — once installed, the repo owns the file, and the
CLI that installed it can also overwrite it.

- `src/components/upstream-policy.json` records what must stay true, enforced by `just check`:
  directory **invariants** (repo-wide rules every file in a directory obeys, so a newly added
  primitive is covered without a manifest edit), per-file **deviations** (intentional
  divergences), and **contracts** (upstream API we depend on and could lose by refactoring
  rather than overwriting — SDK types, Streamdown, `useControllableState`).
...
## AI Elements / Plate foundation rules

### AI Elements

- install into your local codebase and wrap it
- do not spread raw vendor examples across the app
- establish naming and ownership for message rows, citations, loaders, code blocks, etc.

### Plate

- decide the base editor kit early
- document plugin choices
- document serialization/import/export strategy
- decide how AI-assisted editing enters the editor without replacing the editor as canonical truth
...
- AI Elements / Plate boundaries are documented
- sync policy exists
