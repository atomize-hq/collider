...
export const defaultStoryInventoryPath = 'storybook/story-inventory.json';
export const storyInventoryUsage =
...
export function readStoryInventory(target) {
...
export function loadAndValidateStoryInventory(target = defaultStoryInventoryPath) {
  const { absPath, data } = readStoryInventory(target);
  return {
    absPath,
    data,
    errors: validateStoryInventory(data),
  };
}
...
export function validateStoryInventory(data) {
  ...
  if (!Array.isArray(data.components)) {
    errors.push('[CT-9B_STORY_INVENTORY_INVALID_COMPONENTS] components must be an array');
    return errors;
  }
...
function validateComponent(errors, component, index, seenComponentIds) {
  ...
  const validatorKinds = validateValidatorKinds(errors, component.validatorKinds, label);
...
function validateValidatorKinds(errors, validatorKinds, componentLabel) {
  ...
  if (!Array.isArray(validatorKinds)) {
    errors.push(`[CT-9B_STORY_INVENTORY_INVALID_KIND_LIST] ${label} must be an array`);
    return null;
  }
  ...
  for (const [index, kind] of validatorKinds.entries()) {
    ...
    if (typeof kind !== 'string' || kind.length === 0) {
      errors.push(`[CT-9B_STORY_INVENTORY_INVALID_KIND] ${itemLabel} must be a non-empty string`);
      continue;
    }
...
function validateImplementedStoryRefs(errors, implementedStoryRefs, componentLabel) {
  const label = `${componentLabel}.implementedStoryRefs`;
  if (!Array.isArray(implementedStoryRefs)) {
    errors.push(`[CT-9B_STORY_INVENTORY_INVALID_STORY_REF_LIST] ${label} must be an array`);
    return null;
  }
...
function assertPlainObject(errors, value, label) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    errors.push(`[CT-9B_STORY_INVENTORY_INVALID_OBJECT] ${label} must be an object`);
    return false;
  }
...
function validateKeySpec(errors, value, keySpec, label) {
  ...
  for (const key of requiredKeys) {
    if (!actualKeys.includes(key)) {
      errors.push(`[CT-9B_STORY_INVENTORY_MISSING_REQUIRED_KEY] ${label}.${key} is required`);
    }
  }

  for (const key of actualKeys) {
    if (!allowedKeys.includes(key)) {
      errors.push(`[CT-9B_STORY_INVENTORY_UNEXPECTED_KEY] ${label}.${key} is not allowed`);
    }
  }
}
...
function requireLiteral(errors, actual, expected, label) {
  if (actual !== expected) {
    errors.push(`[CT-9B_STORY_INVENTORY_INVALID_LITERAL] ${label} must be ${expected}`);
  }
}
