...
function validateReferenceTree(value, jsonPath, rules) {
  ...
  const error =
    expectObject(value, jsonPath, 'reference nodes must be an object or token reference') ??
    expectValue(
      Object.keys(value).length > 0,
      jsonPath,
      'min-properties',
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    const childError =
      expectIdentifier(key, rules, keyPath, 'reference-tree keys must be lowercase kebab-case') ??
