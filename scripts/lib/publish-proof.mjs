...
export function validatePublishProof(data) {
  ...
  validateKeySpec(
    ...
    data,
    {
      required: ['proofVersion', 'mode', 'artifact', 'destination', 'materialization', 'carrier'],
      optional: [],
    },
    'publishProof'
  );
...
function validateArtifact(errors, artifact) {
  ...
  validateKeySpec(errors, artifact, { required: ['path', 'gitSha'], optional: [] }, 'artifact');
  requireLiteral(errors, artifact.path, publishProofArtifactPath, 'artifact.path');

  if (typeof artifact.gitSha !== 'string' || !shaPattern.test(artifact.gitSha)) {
    errors.push(
...
function validateDestination(errors, destination) {
  if (!assertPlainObject(errors, destination, 'destination must be an object')) {
    return;
  }
  ...
  requireLiteral(errors, destination.name, publishProofPilotName, 'destination.name');
  requireLiteral(errors, destination.figmaFile, publishProofPilotFile, 'destination.figmaFile');
}

function validateMaterialization(errors, materialization) {
  if (!assertPlainObject(errors, materialization, 'materialization must be an object')) {
    return;
  }
  ...
  validateKeySpec(errors, materialization, materializationKeySpec, 'materialization');
  ...
  if (
    ...
    (typeof materialization.notes !== 'string' || materialization.notes.length === 0)
  ) {
    errors.push(
  ...
  if (materialization.status === 'failed' && materialization.notes === undefined) {
    errors.push(
...
function validateCarrier(errors, carrier, mode) {
  ...
  if (typeof carrier.used !== 'boolean') {
    errors.push('[CT-7B_PUBLISH_PROOF_INVALID_CARRIER_USED] carrier.used must be a boolean');
    return;
  }

  if (carrier.used) {
    requireNonEmptyString(errors, carrier.reason, 'carrier.reason');
    requireNonEmptyString(errors, carrier.exitExpectation, 'carrier.exitExpectation');
...
function validateKeySpec(errors, value, keySpec, label) {
  ...
  for (const key of requiredKeys) {
    if (!actualKeys.includes(key)) {
      errors.push(`[CT-7B_PUBLISH_PROOF_MISSING_REQUIRED_KEY] ${label}.${key} is required`);
    }
  }

  for (const key of actualKeys) {
    if (!allowedKeys.includes(key)) {
      errors.push(`[CT-7B_PUBLISH_PROOF_UNEXPECTED_KEY] ${label}.${key} is not allowed`);
    }
  }
}
...
function requireLiteral(errors, actual, expected, label) {
  if (actual !== expected) {
    errors.push(`[CT-7B_PUBLISH_PROOF_INVALID_LITERAL] ${label} must be ${expected}`);
  }
}

function requireNonEmptyString(errors, actual, label) {
  if (typeof actual !== 'string' || actual.length === 0) {
    errors.push(`[CT-7B_PUBLISH_PROOF_INVALID_STRING] ${label} must be a non-empty string`);
  }
}
