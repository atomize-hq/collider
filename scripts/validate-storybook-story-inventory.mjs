...
assert(
  typeof data.inventoryVersion === 'string' && data.inventoryVersion.length > 0,
  'inventoryVersion is required'
);
...
assert(
  typeof data.component.tier === 'string' && data.component.tier.length > 0,
  'component.tier is required'
);
...
for (const [index, story] of data.stories.entries()) {
  assert(
    ...
    `stories[${index}].name is required`
  );
  assert(
    typeof story.kind === 'string' && allowedKinds.has(story.kind),
    `stories[${index}].kind must be one of the allowed values`
  );
